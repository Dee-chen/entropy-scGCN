...
args=commandArgs(T)
setwd("../")
createInput<-function (qur_dir,ref_dir,ref_cluster="cluster",qur_cluster="cluster",genes.file="F"){
...
#################################################################
    ...
    qur_counts<-as_matrix(qur_counts)
...
#在for循环外面对ref_counts进行操作，只需要执行一次Data1.csv
    ...
    print(paste("genes.file是:",genes.file))
    if(genes.file=="F"){
        if(length(inter_genes)>=2000){
            nf=2000
        }else{
            nf=nrow(count_list[[1]])
        }
        print("nf是：")
        print(nf)
        data<-count_list[[1]]
	...
	use.genes<-read.table(genes.file)
...
createInput(args[1],args[2],args[3],args[4],args[5])
